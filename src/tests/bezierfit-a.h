#ifndef BEZIERFIT_A_H
#define BEZIERFIT_A_H
    ...
    gsl_vector * f)
{
  ...
  for (size_t ii = 0; ii < n; ii++) {
    ...
    for (auto const& point : d->target) {
      double const current_distance = Geom::distanceSq(Geom::Point(result.first, result.second), point);
      if (current_distance < best_distance) {
        best_distance = current_distance;
        gsl_vector_set(f, 2*ii, result.first - point.x());
        gsl_vector_set(f, 2*ii+1, result.second - point.y());
      }
    }
...
void get_initial_guess(Geom::CubicBezier& c, std::vector<Geom::Point> const& data) {
  ...
  if (data.size() > 1) {
    ...
    for (size_t ii = 1; ii+1 < data.size(); ++ii) {
      if (initial_t[ii] > 1e-7) {
        double const t = initial_t[ii];
        double const weight = left_right_dist(data, ii);
        tang1 += (data[ii] - front) / t * weight;
        weight1 += weight;
      }
      if (initial_t[ii] < 1-(1e-7)) {
    ...
    if (current_worst_diff < best_worst_diff) {
      best_worst_diff = current_worst_diff;
      c = copy;
      winner = std::string("tangents with distance weight");
    }
  }
...
std::pair<double, double> fit_bezier_icp(Geom::CubicBezier& c, std::vector<Geom::Point> const& data) {
  std::pair<double, double> result(0,0);
  if (data.empty()) {
    return result;
  }
...
*/
  ...
  /* define the function to be minimized */
  fdf.f = bezierfit_icp_f;
  //fdf.df = bezierfit_icp_df;   /* set to NULL for finite-difference Jacobian */
  fdf.df = NULL;   /* set to NULL for finite-difference Jacobian */
  fdf.fvv = NULL;     /* not using geodesic acceleration */
  fdf.n = num_residuals;
  fdf.p = num_params;
